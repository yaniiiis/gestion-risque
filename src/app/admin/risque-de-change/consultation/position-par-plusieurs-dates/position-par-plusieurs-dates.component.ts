...
export class PositionParPlusieursDatesComponent implements OnInit {
    ...
    this.soldeCompteService.findAllDevises().subscribe({
      next: (data) => {
        ...
        allDevises.forEach((d) => {
          this.deviseDropdownList = this.deviseDropdownList.concat({
            item_id: d,
            item_text: d,
          });
  ...
  chosenYearHandler(ev, input) {
    ...
    this.soldeCompteService
      .findSoldeCompteDatesByYear(this.selectedYear)
      .subscribe((data) => {
        allDates = data;
        ...
        allDates.forEach((d) => {
          ...
          this.dateDropdownList = this.dateDropdownList.concat([
            {
              item_id: d,
              item_text: moment(d).format("DD/MM/yyyy"),
            },
          ]);
        });
      });
  ...
  onDeviseItemSelect(item: any) {
    this.dateSelectedItems.forEach((date) => {
      this.soldeCompteService
        .getRatioByDevise(date.item_id, item.item_id)
        .subscribe({
          next: (value) => {
            const a = {} as Ratio;
